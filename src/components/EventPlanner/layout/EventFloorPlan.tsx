...
import {mockDatabase, gridSize, iconLibrary, shapeLibrary} from './constants';
import {lazy} from 'react';
const Stage = lazy(() => import('./static/Stage'));
const BuffetArea = lazy(() => import('./static/BuffetArea'));
const Chats = lazy(() => import('./static/Chats'));
const Entry = lazy(() => import('./static/Entry'));
const Exit = lazy(() => import('./static/Exit'));
const NoEntryZone = lazy(() => import('./static/NoEntryZone'));
const RestRoom = lazy(() => import('./static/RestRoom'));
const ServiceArea = lazy(() => import('./static/ServiceArea'));
const KitchenArea = lazy(() => import('./static/KitchenArea'));
const ReservedArea = lazy(() => import('./static/ReservedArea'));
const Road = lazy(() => import('./static/Road'));
const Parking = lazy(() => import('./static/Parking'));
const WashingPlace = lazy(() => import('./static/WashingPlace'));
const WelcomeDrink = lazy(() => import('./static/WelcomeDrink'));
const Soup = lazy(() => import('./static/Soup'));
const Starter = lazy(() => import('./static/Starter'));
const Toilet = lazy(() => import('./static/Toilet'));
const DustBin = lazy(() => import('./static/DustBin'));
import {api} from '@/utils/axios';
import {useGetSubevent} from '@/lib/react-query/queriesAndMutations/cateror/event';
import {Route} from '@/routes/_app/_event/events.$id';
import toast from 'react-hot-toast';
import {useGetBanquets} from '@/components/Banquet/banquet';
...
const EventFloorPlan: React.FC = () => {
  ...
  const [layoutName, setLayoutName] = useState<string>('New Event Layout');
  ...
  const {data: banquetsResponse} = useGetBanquets();

  const allSubEvents = subEventResponse?.data?.subEvents?.map((subEvent) => ({
    id: subEvent.id,
    name: subEvent.name,
  }));

  const allBanquets = banquetsResponse?.map((banquet) => ({
    id: banquet.id,
    name: banquet.name,
    json: banquet.json,
  }));
  ...
  // ---------------------------------------------------------------
  // 1. Load Sub-Event Layout
  // ---------------------------------------------------------------
  const [loadedLayout, setLoadedLayout] = useState<SavedLayout | null>(null);

  // Load Banquet Layout (ONLY when banquet selected)
  useEffect(() => {
    if (!selectedBanquet?.id) {
      // Optional: Clear canvas if no banquet
      setObjects([]);
      setBoundaries([]);
    ...
    const fetchBanquetLayout = async () => {
      try {
        const res = await api.get(`/cateror/banquet/${selectedBanquet.id}`);
        const jsonStr = res.data?.json;

        if (!jsonStr) {
          toast.info('No saved layout for this banquet');
          setObjects([]);
          setBoundaries([]);
          setLayoutName(`${selectedBanquet.name} – New Layout`);
          return;
        }

        const parsed: SavedLayout = JSON.parse(jsonStr);
        setLoadedLayout(parsed); // This will trigger apply effect
        setLayoutName(parsed.name || selectedBanquet.name);
      } catch (err) {
        console.error('Failed to load banquet layout', err);
        toast.error('Could not load banquet layout');
        setObjects([]);
        setBoundaries([]);
      }
    };
    ...
    fetchBanquetLayout();
  }, [selectedBanquet?.id]); // Only depend on banquet ID

  // ---------------------------------------------------------------
  // 3. Apply Loaded Layout
  // ---------------------------------------------------------------
  useEffect(() => {
  ...
  const saveLayout = useCallback(async () => {
    ...
    const payload = {
      ...
      ...(selectedBanquet?.id && {banquetId: selectedBanquet.id}),
    };
    ...
    mockDatabase.saveLayout(layout);
    setSavedLayouts(mockDatabase.getLayouts());
  }, [layoutName, objects, boundaries, selectedSubEvent, selectedBanquet]);
  ...
  const exportToJSON = useCallback(() => {
    const serializableObjects = objects.map((obj) => ({
      ...obj,
      component: undefined,
      componentType: obj.category === 'static' ? obj.type : undefined,
    }));
    ...
    <div className="flex h-screen w-full overflow-hidden">
      ...
      <div className="flex h-screen flex-1 flex-col overflow-hidden">
        ...
        <div className="mx-4 mb-2">
          ...
          <select
            id="subEventSelect"
            className="border-gray-300 w-full rounded-md border px-3 py-1.5 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-black"
            value={selectedSubEvent?.id || ''}
            onChange={(e) =>
              setSelectedSubEvent(
                allSubEvents?.find((s) => s.id === e.target.value) || null,
              )
            }
          >
        ...
        <div className="mx-4 mb-4">
          ...
          <select
            id="banquetSelect"
            className="border-gray-300 w-full rounded-md border px-3 py-1.5 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-black"
            value={selectedBanquet?.id || ''}
            onChange={(e) => {
              const banquet =
                allBanquets?.find((b) => b.id === e.target.value) || null;
              setSelectedBanquet(banquet);
            }}
          >
            <option disabled value="">
              Select Banquet (Optional)
            </option>
            {allBanquets?.map((banquet) => (
