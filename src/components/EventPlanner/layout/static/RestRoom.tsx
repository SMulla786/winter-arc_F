...
const RestRoom: React.FC<StaticComponentsPanelProps> = ({
  addCustomComponent,
}) => {
  const handleAddComponent = () => {
    const input = prompt(
      'Enter dimensions for Rest Room (Width x Height in ft):',
    );
    if (!input) return;
    ...
    const [w, h] = parts;
    addCustomComponent('restRoom', w * FOOT_TO_PIXEL, h * FOOT_TO_PIXEL);
  };
  ...
  return (
    ...
    >
      ...
      <span className="text-xs font-medium">Rest Room</span>
    </button>
  );
};
