...
const KitchenArea: React.FC<StaticComponentsPanelProps> = ({
  addCustomComponent,
}) => {
  const handleAddComponent = () => {
    const input = prompt(
      'Enter dimensions for Kitchen Area (Width x Height in ft):',
    );
    if (!input) return;
    ...
    const [w, h] = parts;
    addCustomComponent('kitchenArea', w * FOOT_TO_PIXEL, h * FOOT_TO_PIXEL);
  };
  ...
  return (
    ...
    >
      ...
      <span className="text-xs font-medium">Kitchen Area</span>
    </button>
  );
};
