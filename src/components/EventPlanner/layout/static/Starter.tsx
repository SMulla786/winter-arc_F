...
const Starter: React.FC<StaticComponentsPanelProps> = ({
  addCustomComponent,
}) => {
  const handleAddComponent = () => {
    const input = prompt(
      'Enter dimensions for Starter (Width x Height in ft):',
    );
    if (!input) return;
    ...
    const [w, h] = parts;
    addCustomComponent('starter', w * FOOT_TO_PIXEL, h * FOOT_TO_PIXEL);
  };
