...
import {useGetUtensils} from '@/lib/react-query/queriesAndMutations/admin/utensils';
import {useAddUtensilsInventory} from '@/lib/react-query/queriesAndMutations/cateror/utensils';
import React, {useEffect, useState} from 'react';
import Loader from '../common/Loader';
import {
  useAddDisposalInventory,
  useGetDisposals,
...
const DisposalInventory = () => {
  ...
  const restriction = user?.employeeRestriction?.disposalInventoryPage;
  ...
  console.log('disposal inventory', disposalsApiData);
  ...
  const {mutateAsync: addDisposalInventory, isPending} =
    useAddDisposalInventory();
  ...
  const handleSavePrices = async () => {
    const pricesArray = (disposalsApiData?.data ?? []).map((item) => ({
      disposalId: item.id,
      inventory: Number(inventories[item.id] ?? item.inventory),
    }));
    console.log('Submitting data:', pricesArray);

    try {
      await addDisposalInventory(pricesArray);
      setPrices({});
      setInventories({});
    } catch (error) {
  ...
  return (
    <div className="">
      ...
      <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <h2 className="mb-4 text-xl font-bold">Disposal Inventory</h2>
