...
const ExternalHistoryLineItemRow: React.FC<LineItemRowProps> = ({
  item,
  itemIndex,
  isCompletedView,
  vendorOptions,
    ...
    >
      <td className="px-4">
        {isFirstOfMaterial ? (
          ...
          <div className="text-gray-400 text-sm italic">{/* ↳ */}</div> // optional visual cue
        )}
      </td>
      <td>
        {isFirstOfMaterial ? (
          <span className="inline-flex items-center gap-1 whitespace-nowrap rounded bg-blue-50 px-2 py-1 text-xs font-medium dark:bg-transparent dark:text-white">
      ...
      <td className="px-3">
        {isCompletedView ? (
          ...
          <>
            ...
            {/* Show error message below select */}
            {item.requiredErrors &&
              !isCompletedView &&
              item.requiredErrors.some((err) => err.includes('Vendor')) && (
                <div className="mt-1 flex items-center gap-1 text-xs text-red-500">
...
export default ExternalHistoryLineItemRow;
