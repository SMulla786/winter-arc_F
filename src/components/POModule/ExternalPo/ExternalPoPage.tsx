...
import ExternalPoTable from './ExternalPoTable';
import ExternalPoHeaderButtons from './ExternalPoHeaderButtons';

export type props = {
  id: string;
};
const ExternalPoPage: React.FC<props> = ({id}) => {
  ...
  useEffect(() => {
    ...
    // Check if rawMaterialOrderData exists and has data
    if (rawMaterialOrderData?.data) {
      ...
      if (Array.isArray(rawMaterialOrderData.data)) {
        ...
        itemsToProcess = rawMaterialOrderData.data;
      } else if (
        rawMaterialOrderData.data.formatedRawMaterials &&
        Array.isArray(rawMaterialOrderData.data.formatedRawMaterials)
      ) {
        // If it has formatedRawMaterials property
        itemsToProcess = rawMaterialOrderData.data.formatedRawMaterials;
      } else if (typeof rawMaterialOrderData.data === 'object') {
        // If it's an object, try to get values or handle as single item
        itemsToProcess = [rawMaterialOrderData.data];
      }
  ...
  const processRawMaterialOrderData = useCallback(
    (rawMaterials: any): LineItem[] => {
      ...
      if (Array.isArray(rawMaterials)) {
        materialsArray = rawMaterials;
      } else if (
        rawMaterials?.formatedRawMaterials &&
        Array.isArray(rawMaterials.formatedRawMaterials)
      ) {
        materialsArray = rawMaterials.formatedRawMaterials;
      } else if (rawMaterials && typeof rawMaterials === 'object') {
      ...
      const lastPriceMap = extractLastPrices(materialsArray);
      ...
      materialsArray.forEach((item) => {
        ...
        if (!rawMaterialId) return; // Skip items without ID
  ...
  const handleSubmitOrder = (isDraft: boolean = true) => {
    ...
    //   toast.error(
    //     'Please fix quantity over-allocation errors before submitting',
    //   );
    //   // Optionally re-validate and mark rows with error (but don't show required field errors yet)
    //   if (isDraft) setIsSavingDraft(false);
    //   return;
    // }
  ...
  return (
    <div>
      <div className="mt-2 rounded-md px-2 sm:px-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          {/* Left section with title and status badges */}
          <div className="flex flex-wrap items-center gap-2 sm:gap-4">
            <h1 className="text-lg font-bold text-graydark dark:text-gray sm:text-xl">
              Purchase Order
            </h1>
            <div className="flex flex-wrap gap-2">
              {isViewMode && currentPoStatus === 'COMPLETED' && (
                <span className="inline-flex items-center rounded-full bg-green-500 px-2 py-1 text-xs font-semibold text-white sm:px-3">
                  COMPLETED
                </span>
              )}
              {currentPoStatus === 'PARTIAL' && (
                <span className="inline-flex items-center rounded-full bg-yellow-500 px-2 py-1 text-xs font-semibold text-white sm:px-3">
                  DRAFT
                </span>
              )}
              {!currentPoStatus && (
                <span className="inline-flex items-center rounded-full bg-blue-500 px-2 py-1 text-xs font-semibold text-white sm:px-3">
                  NEW
                </span>
              )}
            </div>
