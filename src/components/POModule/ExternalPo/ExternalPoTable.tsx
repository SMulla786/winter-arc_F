...
import ExternalCategoryHeader from './ExternalCategoryHeader';
import ExternalLineItemRow from './ExternalLineItemRow';
import ExternalBreakDownRow from './ExternalBreakDownRow';
...
interface EventPoTableProps {
  ...
  getMainRowQuantity?: (rawmaterialId: string) => number; // Function to get main row quantity
}

const ExternalPoTable: React.FC<EventPoTableProps> = ({
  groupedItems,
  expandedCategories,
  expandedBreakdowns,
  categoryConfigs,
  ...
  addBreakdownRow,
  removeBreakdownRow,
  getMainRowQuantity, // Changed from mainRowQuantity to a function
  toggleCategory,
  toggleBreakdown,
}) => {
                  ...
                  {expandedCategories[category] &&
                    groupedItems[category]
                      ...
                      .map((item, itemIndex, array) => {
                        ...
                        // Get main row quantity for this material
                        const mainRowQuantity = getMainRowQuantity
                          ? getMainRowQuantity(item.rawmaterialId)
                          : item.quantity || 0;
                            ...
                            {isBreakdownExpanded &&
                              breakdownItems.map(
                                (breakdown, breakdownIndex) => {
                                  // Calculate total breakdown quantity for this material
                                  const totalBreakdownQuantity =
                                  ...
                                  return (
                                    <ExternalBreakDownRow
                                      ...
                                      availableQuantity={mainRowQuantity}
                                      currentTotalBreakdownQuantity={
                                        totalBreakdownQuantity
                                      }
                                    />
                                  );
                                },
...
export default ExternalPoTable;
