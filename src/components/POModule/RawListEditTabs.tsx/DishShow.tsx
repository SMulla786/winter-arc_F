...
import {FiDownload, FiArrowLeft, FiCalendar} from 'react-icons/fi';
...
const DishShow: React.FC<Props> = ({listId, onBack}) => {
  const {data: rawListData, isLoading} = useGetRawListHistoryById(listId);
  const {user} = useAuthContext();

  const originalDishes = rawListData?.data?.dishList;

  const [dishes, setDishes] = useState<DishItem[]>(() => {
    if (!originalDishes) return [];
    ...
    originalDishes.forEach((dish) => {
      ...
      if (existing) {
        dishMap.set(dish.dishId, {
          ...existing,
          preparation: Number(existing.preparation) + Number(dish.preparation),
          quantity: Number(
    ...
    viewDishes?.forEach((dish) => {
      ...
      const subeventNames = Array.isArray(dish.subeventName)
        ? dish.subeventName.map((name) => ({
            ...
            preparation: dish.preparation || 'N/A',
            unit: dish.unit || 'pcs',
          }))
        : dish.subeventName
          ? [
  ...
  const handleExportPDF = async () => {
    ...
    reportContainer.style.padding = '10px 15px';
    reportContainer.style.background = 'white';
    reportContainer.style.fontFamily = 'Arial, sans-serif';
    reportContainer.style.fontSize = '11px';
    ...
    headerWrapper.style.padding = '4px';
    ...
    <h1 style="margin:0; font-weight:800; font-size:24px; color:black;">
      ${user?.fullname || 'Caterer Name'}
    </h1>
    <div style="background:#0D47A1; height:1px; margin:3px auto; width:80%;"></div>
  ...
  `;
    ...
    <h2 style="font-size: 16px; font-weight: bold; margin:0; color:black;">
      Total Dish Count Report ${rawListData?.data?.listNo ? ` - List #${rawListData.data.listNo}` : ''}
    </h2>
    <div style="margin-bottom: 5px; color:black; font-size: 11px;">
      ...
      ${
        rawListData?.data
          ? `
            <span style="display: inline-block; margin: 0 4px; color:black;">
              <strong>From:</strong> ${formatDisplayDate(rawListData.data.from)}
            </span>
            <span style="display: inline-block; margin: 0 4px; color:black;">
              <strong>To:</strong> ${formatDisplayDate(rawListData.data.to)}
            </span>
          `
          : ''
      }
  ...
  `;
    ...
    statsSection.style.margin = '8px 0';
  ...
  `;
    ...
    signatureLine.style.marginTop = '15px';
    signatureLine.style.borderTop = '1px solid #0D47A1';
    signatureLine.style.paddingTop = '8px';
  ...
  if (!rawListData?.data) {
    return (
      <div className="flex h-64 flex-col items-center justify-center">
        ...
        <p className="text-gray-500 mt-4 text-lg">No data found</p>
      </div>
    );
  }
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-start gap-3 md:items-center">
            {onBack && (
              <button
                onClick={onBack}
                className="rounded-lg p-2 transition-colors hover:bg-blue-700/30"
              >
            ...
            <div className="text-white">
              <h1 className="text-2xl font-bold">
                Dish Count Results{' '}
                {rawListData?.data?.listNo
                  ? `(List #${rawListData.data.listNo})`
                  : ''}
              </h1>
              <div className="mt-2 flex flex-wrap items-center gap-4">
                <div className="flex items-center gap-2 text-sm text-blue-100">
                  ...
                  <span>
                    {formatDisplayDate(rawListData.data.from)} -{' '}
                    {formatDisplayDate(rawListData.data.to)}
                  </span>
                </div>
              </div>
            </div>
      ...
      {selectedDish && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-2 sm:p-4">
          <div className="dark:bg-gray-800 relative flex max-h-[90vh] w-full max-w-6xl flex-col rounded-lg bg-white shadow-2xl">
            ...
            <div className="dark:bg-gray-800 dark:border-gray-700 flex flex-shrink-0 items-center justify-between border-b bg-white px-3 py-2 sm:px-4 sm:py-3">
                    ...
                    <div className="space-y-3">
                      {dishEventsMap[selectedDish.dish].map(
                        (event, eventIndex) => (
                          <div
                            key={eventIndex}
                            className="border-gray-200 dark:border-gray-700 overflow-hidden rounded border"
                          >
                            {/* Event Header */}
                              ...
                              <GenericTable
                                data={event.subeventNames.map(
                                  (subevent, index) => ({
                                    ...
                                    preparation: subevent.preparation,
                                    unit: subevent.unit,
                                  }),
                                )}
                                columns={[
