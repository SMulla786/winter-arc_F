...
const DisplayUtensilCategoryCat: React.FC = () => {
  ...
  const {data, isPending, isSuccess} = useGetUtensilCategories();
      ...
      customUI: ({onClose}) => (
        <div className="fixed inset-0 z-20 flex items-center justify-center bg-meta-4 bg-opacity-50">
          <div className="w-full max-w-md rounded-lg bg-white p-6 text-black shadow-md dark:bg-meta-4 dark:text-white">
            ...
            <p className="mb-6">
              Are you sure you want to delete utensil category
  ...
  const handleExportUtensilsCategory = () => {
    ...
    if (exportData && exportData.length > 0) {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Utensils Category');

      XLSX.writeFile(wb, 'UtensilsCategory.xlsx');
    }
  };
