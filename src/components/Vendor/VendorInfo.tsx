...
import {
  useGetManPowerVendorHistoryById,
  useSaveManPowerVendorAdvancePay,
  useSaveManPowerVendorPay,
  useSaveMultiManPowerVendorPay,
} from '@/lib/react-query/queriesAndMutations/cateror/vendorManpower';
import {Route} from '@/routes/_app/_vendor/menpowervendor.$id';
...
const ManPowerVendorHistory: React.FC = () => {
  const {id} = Route.useParams();
  const {data: history} = useGetManPowerVendorHistoryById(id);
  ...
  const {mutateAsync: payManPowerVendor} = useSaveManPowerVendorPay();
  const navigate = useNavigate();

  const {mutate: payManPowerVendorAdvance} = useSaveManPowerVendorAdvancePay(
    id as string,
  );
  const {mutate: multiPayManPowerVendor} = useSaveMultiManPowerVendorPay();
    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        ...
        <button
          ...
          onClick={() => {
            navigate({to: `/manpowervendorhistoryshow/${id}`});
          }}
        >
          View History
        </button>
      ...
      <div className="dark:bg-gray-900 overflow-x-auto rounded-lg bg-white p-4 shadow-md dark:bg-black">
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
          <h2 className="text-gray-800 dark:text-gray-100 text-lg font-semibold">
            {history?.vendor?.name || 'N/A'} Vendor Work History
          </h2>
                    ...
                    <td className="text-center">
                      <div className="flex items-center justify-center gap-2 p-2">
                        ...
                        <button
                          onClick={() =>
                            navigate({
                              to: `/manpowereventhistoryshow/${event.eventId}`,
                              state: {vendorId: id},
                            })
                          }
...
export default ManPowerVendorHistory;
