...
import {Route} from '@/routes/_app/_displayvendor/displayvendorhistoryshow.$id';
import {
  Calendar,
  IndianRupee,
  MapPin,
...
import {useGetDisplayVendorAllHistory} from '@/lib/react-query/queriesAndMutations/cateror/displayVendor';
import {FaArrowLeft} from 'react-icons/fa';
import {useRouter} from '@tanstack/react-router';

const DisplayVendorHistoryShow = () => {
  const {id} = Route.useParams();
  const route = useRouter();
  const {data: history, isLoading, isError} = useGetDisplayVendorAllHistory(id);
  ...
  const filtered = useMemo(() => {
    ...
    return arr
      ...
      .filter((it: any) => {
        if (!q) return true;
        // Adjusted for display structure: check across all eventVendorHistories
        const matches = it.eventVendorHistories?.some((evh: any) => {
          const subEvent = evh.subEventDisplay?.subEvent;
          return (
            String(it.type).toLowerCase().includes(q) ||
            String(evh.subEventDisplay?.displayId || '')
              .toLowerCase()
              .includes(q) ||
    ...
    <div className="dark:bg-dark min-h-screen w-full">
      <div className="mx-auto w-full">
        <div className="mb-2 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <h1 className="text-gray-900 flex flex-row items-center gap-2 text-lg font-semibold">
            <FaArrowLeft
                      ...
                      <div className="border-gray-200 mt-2 border-t">
                        {item.eventVendorHistories.map(
                          (evh: any, index: number) => {
                            ...
                            const displayItem = evh.subEventDisplay;
                            ...
                            return (
                              ...
                              >
                                ...
                                {/* DISPLAY DETAILS - Adjusted table for display items */}
                                <div className="mt-3">
                                  <div className="overflow-x-auto">
                                    <div className="border-gray-200 w-full border-t text-xs">
                                      <div className="bg-gray-100 text-gray-700 grid grid-cols-5 text-left">
                                ...
                                {/* Additional Info Section - To show all info, including less common fields */}
                                {/* <div className="mt-4 pt-4 border-t border-gray-200">
                                  <h5 className="text-gray-800 text-xs font-semibold uppercase tracking-wide mb-2">
                                    Additional Details
                                  </h5>
                                  <div className="grid grid-cols-1 gap-2 text-xs text-gray-600 md:grid-cols-2">
                                    <div><strong>ID:</strong> {displayItem?.id || '-'}</div>
                                    <div><strong>Created:</strong> {new Date(displayItem?.createdAt).toLocaleString()}</div>
                                    <div><strong>Updated:</strong> {new Date(displayItem?.updatedAt).toLocaleString()}</div>
                                    <div><strong>SubEvent ID:</strong> {displayItem?.subEventId || '-'}</div>
                                    <div><strong>Display Vendor ID:</strong> {displayItem?.displayVendorId || '-'}</div>
                                    {subEvent && (
                                      <>
                                        <div><strong>SubEvent Note:</strong> {subEvent?.note || 'None'}</div>
                                        <div><strong>Final Amount:</strong> {formatCurrency(subEvent?.finalAmount)}</div>
                                        <div><strong>Expected Cost:</strong> {formatCurrency(subEvent?.expectedCost)}</div>
                                        <div><strong>Profit:</strong> {formatCurrency(subEvent?.profit)}</div>
                                        <div><strong>Sitting People:</strong> {subEvent?.sittingPeople || '-'}</div>
                                        <div><strong>Buffet People:</strong> {subEvent?.buffetPeople || '-'}</div>
                                        <div><strong>Dish Calculation:</strong> {subEvent?.dishCalculation ? 'Yes' : 'No'}</div>
                                        <div><strong>Billing Cost:</strong> {formatCurrency(subEvent?.biilingCost)}</div>
                                        <div><strong>Discount Given:</strong> {formatCurrency(subEvent?.discountGiven)}</div>
                                      </>
                                    )}
                                    {event && (
                                      <>
                                        <div><strong>Event Status:</strong> {event?.status || '-'}</div>
                                        <div><strong>Event isActive:</strong> {event?.isActive ? 'Yes' : 'No'}</div>
                                        <div><strong>Quotation Amount:</strong> {formatCurrency(event?.quotationAmount)}</div>
                                        <div><strong>Paid Amount:</strong> {formatCurrency(event?.paidAmount)}</div>
                                        <div><strong>Balance:</strong> {formatCurrency(event?.balance)}</div>
                                        <div><strong>Discount Given (Event):</strong> {formatCurrency(event?.discountGiven)}</div>
                                      </>
                                    )}
                                  </div>
                                </div> */}
...
export default DisplayVendorHistoryShow;
