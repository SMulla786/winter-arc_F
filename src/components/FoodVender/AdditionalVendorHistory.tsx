...
} from '@/lib/react-query/queriesAndMutations/cateror/additionalVendor';
import {Route} from '@/routes/_app/_vendor/additionalvendorhistory.$id';
...
const AdditionalVendorHistory: React.FC = () => {
  const {id} = Route.useParams();
  const {data: history} = useGetAdditionalVendorHistoryById(id);
  ...
  const {mutateAsync: payAdditionalVendor} = useSaveAdditionalVendorPay();
  const navigate = useNavigate();

  const {mutate: payAdditionalVendorAdvance} =
    useSaveAdditionalVendorAdvancePay(id as string);
  const {mutate: multiPayAdditionalVendor} = useSaveMultiAdditionalVendorPay();
    ...
    <div>
      <div className="mb-4">
        <button
          ...
          onClick={() => navigate({to: '/vendormanagement?tab=additional'})}
        >
          ← Back
        </button>
      </div>
                    ...
                    <td className="p-2 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button
                          onClick={() =>
                            navigate({
                              to: `/additinalvendoreventhistory/${event.eventId}`,
                              state: {
                                vendorId: id,
...
export default AdditionalVendorHistory;

// import React from 'react'

// const AdditionalVendorHistory = () => {
//   return (
//     <div>
//       asdhfjsadghfj
//     </div>
